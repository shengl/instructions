...
1.**\[展开片数\]**：根据当前编织花型的宽度设定

2.**\[间隔针数\]**：数值越大，每片编织区的间隔距离越大，根据需要设定

3.**\[展开篇幅\]**：  
   \[当前花型的宽度\]×\[展开片数 \]+ \[间隔针数\]×\[展开片数 -1\] + \[起针点 -1\]

4.**\[废纱纱嘴使能\]**：  
    **打开**时，每片均使用当前花型默认的起编织废纱功能的编号纱嘴；  
    **关闭**时，可以自己设定每片起编织废纱功能的纱嘴编号。

5.**\[纱嘴共用\]**：  
    **打开**时机头系统始终使用一套纱嘴进行编织，编制片交替时纱嘴也不进行更换，每行编织过程中纱嘴不会被卸下；  
    **关闭**时可根据实际需要设置每片编织使用的纱嘴，且每片使用纱嘴不可重复，编制片交替时机头会卸下原有纱嘴，进入下一片编制片时带起新一组纱嘴。
