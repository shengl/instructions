...
## 供客户查看、操作系统主要设置菜单的快捷窗口。

**触摸**：在运行监控画面点击该键弹出\[功能菜单\]，退出请点击右上角\[×\]退回至\[编织运行\]界面，或者点击**\[ESC：退出到主菜单\]**退至主界面。  
**按键**：在运行监控画面按 **？** 键弹出\[功能菜单\]，按 **Esc** 键退出当前界面返回至\[编织运行\]界面。
...
**触摸**：点击 **\[0：多片展开设置\]** 图标，显示为黄色字体，弹出\[多片展开设置\]界面，根据当前工作花型的宽度填写\[展开片数\]和\[间隔针数\]，选择是否打开\[废纱纱嘴使能\]和\[纱嘴共用\]功能，同时在蓝色参数框中输入需要使用的纱嘴编号。点击右上角\[×\]退回至\[编织运行\]界面。  
**按键**：方向键移动光标选择 **\[0：多片展开设置\]** 图标，按 **Enter** 键进入\[多片展开设置\]界面，通过方向键移动光标选择需要修改的参数框，按回车键弹出小键盘，输入需要使用的纱嘴编号，再次按 **Enter** 键完成输入，按 **Esc** 键退出。
...
**2. \[间隔针数\]**：设定各片之间的间隔针数，以花型最大幅宽计算。

**3. \[展开篇幅\]**：  
\[当前花型的宽度\]×\[展开片数 \]+ \[间隔针数\]×\[展开片数 -1\] + \[起针点 -1\]

**4. \[废纱纱嘴使能\]**：  
**打开**时，各片共用起始编织废纱纱嘴。 **关闭**时，使用每片设定的初始编织纱嘴。

**5. \[纱嘴共用\]**：  
**打开**：将各个展开片合成为一片，同时使用基准片纱嘴进行编织。

* 此种编织模式主要用于花型幅宽变化极小，例如裁剪式斜面织物编织等，片间隔不能设定过大否则有无法正常垫纱的情况。

**关闭**：各片使用表中设定的纱嘴编织。

**6.** \[展开片数\]发生变化时，退出当前界面会弹出提示框：**请归零机器，使多片设置修改生效**，点击提示框退回至\[编织运行\]界面。
...
**按键**：方向键移动光标选择**\[F：指定行停车\]**图标，按 **Enter** 键弹出\[指定行停车\]提示框，通过方向键移动光标选择需要修改的参数框，按 **Enter** 键弹出小键盘，根据需求输入停车行数，再次按 **Enter** 键完成输入，按 **Esc** 键退出。
